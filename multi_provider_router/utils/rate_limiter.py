...
class RateLimiter:
    ...
    def __init__(self):
        ...
        self._local_counters: Dict[ProviderType, Dict[str, deque]] = defaultdict(lambda: {
            'minute': deque(),
            'hour': deque(),
            'day': deque()
        })
    ...
    def get_rate_limit_rule(self, provider: ProviderType, user_id: Optional[str] = None) -> RateLimitRule:
    ...
    async def _check_redis_rate_limit(
        ...
        now = int(time.time())
        minute_key = f"rate_limit:{provider.value}:{user_id or 'global'}:minute"
        hour_key = f"rate_limit:{provider.value}:{user_id or 'global'}:hour"
        day_key = f"rate_limit:{provider.value}:{user_id or 'global'}:day"
    ...
    def _check_local_rate_limit(
        ...
        now = time.time()
        counters = self._local_counters[provider]
    ...
    async def reset_rate_limits(
        ...
        if self._redis:
            patterns = [
                f"rate_limit:{provider.value}:{user_id or 'global'}:*"
            ]
